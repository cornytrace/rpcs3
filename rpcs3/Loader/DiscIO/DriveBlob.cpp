...
#include "Loader\DiscIO\Blob.h"
#include "Loader\DiscIO\DriveBlob.h"
...
DriveReader::DriveReader(const std::string& drive)
{
  ...
  SetSectorSize(2048);
#ifdef _WIN32
  auto const path = utf8_to_wstring(std::string("\\\\.\\") + drive);
  ...
  else { LOG_NOTICE(LOADER, "Load from Blu-ray failed or no disc in drive %s", drive.c_str()); }
}
...
bool DriveReader::ReadMultipleAlignedBlocks(u64 block_num, u64 num_blocks, u8* out_ptr)
{
  ...
  if (!SetFilePointerEx(m_disc_handle, offset, nullptr, FILE_BEGIN) ||
      !ReadFile(m_disc_handle, out_ptr, static_cast<DWORD>(GetSectorSize() * num_blocks),
                &bytes_read, nullptr))
  {
    fmt::throw_exception("Disc Read Error");
    return false;
  }
